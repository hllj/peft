...
from layer import ResLoraLinear
import torch
...
class ResLoraModel(nn.Module):
    def __init__(self, model: PreTrainedModel, config: ResLoraConfig, epochs: int = -1, model_name="llama"):
        super().__init__()

        if config.pre_num == -1:
            config.pre_num = 999
        self.config = config
        model.config.config = config.to_json_string()
        self.config = model.config
        self.wrapped_model = model
        self.target_modules = set(config.target_modules.lower().split("."))
        ...
        for name, module in self.wrapped_model.named_modules():
            ...
            if isinstance(module, nn.Linear):
                ...
                if temp_flag == None:
                    continue
                new_module = ResLoraLinear(module, config, name)
    ...
    def calculate_froc(self):
        if self.config.res_flag == 1 or self.config.res_flag == 3:
    ...
    def load_checkpoint(model_dir: str, lora_dir=""):
        ...
        if lora_dir != "":
            ...
            with open(f"{lora_dir}/{LORA_STATIC_CONFIG}", "r") as f:
                config = json.load(f)
            config = ResLoraConfig(**config)
            model = ResLoraModel(model, config)
            missing, unexpected = model.wrapped_model.load_state_dict(reslora, strict=False)
            if config.start_index < config.lora_num - 1:
                model.new_epoch()
            else:
    ...
    def load_full_checkpoint(model_dir: str):
        ...
        with open(f"{model_dir}/config.json", "r") as f:
            config = ResLoraConfig(**json.load(f)["config"])
